...
WNW = args.ew # energy window for the plot
PTS = args.ps # point size
IN_FILE = args.i # name of input file generated by BP-energy_slices.py
#--------------------------------------------------------------------------------------------------------------------------
...
bc1 = params['BC1_topmost_layer'] # bc value for V at the top-most layer
...
#--------------------------------------------------------------------------------------------------------------------------
def ESlicesPlot(x, y, z, Cxy, Cyz, Czx, face, data0):
...
printlog('BinPo, TB-Poisson Solver for 2DES\n'\
 ...
 'You should have received a copy of the GNU General Public License\n'\
 'along with BinPo. See ~/COPYING file or <https://www.gnu.org/licenses/>.')
# loading the output of BP-energy_slices.py
if IN_FILE == 'default':
    try:
    ...
    try:
        ...
        printlog('The input file could not be loaded', level = 'e')
# unfolding all values from output of BP-energy_slices.py        
...
printlog('\n')
if manifold != 't2g' and data['ENERGY_PLOTTER']['orbital_char'] == True:
    printlog('\n')
    printlog("Energy slices calculation with projections onto an arbitrary MLWF is not implemented yet. "\
             "Please, change to orbital_char == no to visualize the energy slice. ", 'e')
    printlog('\n')

# plotting the energy slice    
ESlicesPlot(x, y, z, Cxy, Cyz, Czx, face, data['ENERGY_PLOTTER'])
